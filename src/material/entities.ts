...
import { Cardinal, Denominator, from, INITIAL, to, zeroAndPositiveIntegers } from '@musical-patterns/utilities'
        ...
        return equalDivisions.reduce(
            (accumulatingEntities: Entity[], equalDivision: Denominator): Entity[] => {
                const entitiesForEqualDivision: Entity[] = zeroAndPositiveIntegers
                    .slice(from.Ordinal(INITIAL), from.FractionalPart(equalDivision))
                    .map((step: number) => ({
                        noteSpecs: [
                            buildNoteSpec(
